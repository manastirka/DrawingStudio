...
# App listens on 19100 by default; we cannot change port without code.
# Use default 19100 for smoke against a live app, or start app ourselves.
APP_PORT=19100
if lsof -iTCP:"$APP_PORT" -sTCP:Listen >/dev/null 2>&1; then
  ...
  for i in $(seq 1 30); do
    if curl -sf --max-time 1 "http://127.0.0.1:${APP_PORT}/api/status" >/dev/null; then
...
echo "== status =="
STATUS=$(curl -sf --max-time 3 "$BASE/api/status")
...
STATUS2=$(curl -sf --max-time 3 "$BASE/api/status")
...
echo "== undo =="
curl -sf --max-time 3 -X POST "$BASE/api/undo" >/dev/null

STATUS3=$(curl -sf --max-time 3 "$BASE/api/status")
