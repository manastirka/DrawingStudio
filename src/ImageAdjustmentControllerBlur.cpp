...
// ImageAdjustmentController blur + auto-enhance (refactor E24).

// Preview helpers (internal) — downsample + debounce for real-time blur UI
namespace {
    ...
    // Debounce helper for smooth slider interaction (no Q_OBJECT — lambda timer)
    class DebouncedUpdate {
    public:
        explicit DebouncedUpdate(int delayMs = 50)
            : m_timer(new QTimer())
        {
            m_timer->setSingleShot(true);
            m_timer->setInterval(delayMs);
        }
        ~DebouncedUpdate() { delete m_timer; }
        DebouncedUpdate(const DebouncedUpdate&) = delete;
        DebouncedUpdate& operator=(const DebouncedUpdate&) = delete;

        template<typename F>
        void setCallback(F &&f) {
            QObject::disconnect(m_timer, &QTimer::timeout, nullptr, nullptr);
            QObject::connect(m_timer, &QTimer::timeout, m_timer, std::forward<F>(f));
        }

        void trigger() { m_timer->start(); }
...
// --- showGaussianBlur ---
void ImageAdjustmentController::showGaussianBlur()
{
    ...
    if (!selectedImage) {
        QMessageBox::information(m_host.dialogParent, "Gaussian Blur", "Please select an image first");
        return;
    }
    ...
    debouncer->setCallback([&]() { applyBlur(true);  // Preview mode
     });
    ...
    if (dialog.exec() != QDialog::Accepted) {
        ...
        m_host.canvas->update();
        if (m_host.setStatusText) m_host.setStatusText("Gaussian blur cancelled");
    } else {
        // Apply at full resolution on accept
        applyBlur(false);
        if (m_host.setStatusText) m_host.setStatusText("✓ Gaussian blur applied");
    }
...
// --- showMotionBlur ---
void ImageAdjustmentController::showMotionBlur()
{
    if (!m_host.canvas) return;
    ...
    if (!selectedImage) {
        QMessageBox::information(m_host.dialogParent, "Motion Blur", "Please select an image first");
        return;
    }
    ...
    debouncer->setCallback([&]() { applyMotionBlur(true);  });
    ...
    if (dialog.exec() != QDialog::Accepted) {
        selectedImage->setImage(originalImage);
        m_host.canvas->update();
        if (m_host.setStatusText) m_host.setStatusText("Motion blur cancelled");
    } else {
        applyMotionBlur(false);  // Full resolution on accept
        if (m_host.setStatusText) m_host.setStatusText("✓ Motion blur applied");
    }
...
// --- showRadialBlur ---
void ImageAdjustmentController::showRadialBlur()
{
    if (!m_host.canvas) return;
    ...
    if (!selectedImage) {
        QMessageBox::information(m_host.dialogParent, "Radial Blur", "Please select an image first");
        return;
    }
    ...
    debouncer->setCallback([&]() { applyRadialBlur(true);  });
    ...
    if (dialog.exec() != QDialog::Accepted) {
        selectedImage->setImage(originalImage);
        m_host.canvas->update();
        if (m_host.setStatusText) m_host.setStatusText("Radial blur cancelled");
    } else {
        applyRadialBlur(false);  // Full resolution on accept
        if (m_host.setStatusText) m_host.setStatusText("✓ Radial blur applied");
    }
...
// --- showBokehBlur ---
void ImageAdjustmentController::showBokehBlur()
{
    if (!m_host.canvas) return;
    ...
    if (!selectedImage) {
        QMessageBox::information(m_host.dialogParent, "Bokeh Blur", "Please select an image first");
        return;
    }
    ...
    connect(slider, &QSlider::valueChanged, [debouncer]() { debouncer->trigger(); });
    debouncer->setCallback([&]() { applyBokehBlur(true);  });
    ...
    if (dialog.exec() != QDialog::Accepted) {
        selectedImage->setImage(originalImage);
        m_host.canvas->update();
        if (m_host.setStatusText) m_host.setStatusText("Bokeh blur cancelled");
    } else {
        applyBokehBlur(false);  // Full resolution on accept
        if (m_host.setStatusText) m_host.setStatusText("✓ Bokeh blur applied");
    }
...
// --- showSurfaceBlur ---
void ImageAdjustmentController::showSurfaceBlur()
{
    if (!m_host.canvas) return;
    ...
    if (!selectedImage) {
        QMessageBox::information(m_host.dialogParent, "Surface Blur", "Please select an image first");
        return;
    }
    ...
    debouncer->setCallback([&]() { applySurfaceBlur(true);  });
    ...
    if (dialog.exec() != QDialog::Accepted) {
        selectedImage->setImage(originalImage);
        m_host.canvas->update();
        if (m_host.setStatusText) m_host.setStatusText("Surface blur cancelled");
    } else {
        applySurfaceBlur(false);  // Full resolution on accept
        if (m_host.setStatusText) m_host.setStatusText("✓ Surface blur applied");
    }
...
// --- autoEnhanceImage ---
void ImageAdjustmentController::autoEnhanceImage()
{
    if (!m_host.canvas) return;
    ...
    if (!selectedImage) {
        QMessageBox::information(m_host.dialogParent, "Auto Enhance", "Please select an image first");
        return;
    }
    ...
    m_host.canvas->update();
    if (m_host.setStatusText) m_host.setStatusText("✓ Image auto-enhanced");
}
